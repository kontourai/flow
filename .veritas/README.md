...
This repo was bootstrapped for `flow` with a conservative starter kit for agent-guided development.
...
- Repo kind: `application`
- Evidence Check: `npm test && npm run typecheck && npm run docs:check && npm run check:schemas`
...
`npm test && npm run typecheck && npm run docs:check && npm run check:schemas`
