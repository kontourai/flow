...
Current runtime note: v0.1 Resource Contract support is implemented only for authored Flow Definition and authored Flow Project Config inputs. Flow Run state, gate evidence manifests, Flow Reports, transition validation results, console projections, Flow Agents artifacts, and Builder Kit adapters remain flat or product-native contracts until a future migration slice changes them deliberately.
...
- Pickup Probe: `/Users/brian/dev/github/kontourai/flow-agents/.flow-agents/flow-contract-audit-pickup/flow-contract-audit-pickup--pickup-probe.md`.
- Plan: `/Users/brian/dev/github/kontourai/flow-agents/.flow-agents/flow-contract-resource-audit/flow-contract-resource-audit--plan.md`.
- Idea-to-backlog source artifact: `NOT_VERIFIED`. The named local idea-to-backlog source artifact was not found locally.
...
| Flow Run / state | Flow core | `.flow/runs/<run-id>/state.json` with `schema_version`, `run_id`, `definition_id`, `status`, `current_step`, `gate_outcomes`, `transitions`, `exceptions` | Durable observed run state for continuation, CLI, agents, reports, and future consoles | `migrate` | Run state is observed process state, but v0.1 keeps the persisted contract flat at `state.json`. A future migration should map `metadata` to run identity and labels, `spec` to the definition reference and subject/params, and `status` to current step, gate outcomes, transitions, exceptions, next action, and conditions. | `README.md`, `CONTEXT.md`, `schemas/flow-run.schema.json`, `src/index.js` |
